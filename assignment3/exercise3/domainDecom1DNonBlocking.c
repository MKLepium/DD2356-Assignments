...
int main(int argc, char *argv[]) {
    ...
    // Send left ghost cell
    MPI_Isend(&f[1], 1, MPI_DOUBLE, left, 0, MPI_COMM_WORLD, &requests[0]);
    // Receive right ghost cell
    MPI_Irecv(&f[nxn_loc - 1], 1, MPI_DOUBLE, right, 0, MPI_COMM_WORLD, &requests[1]);

    // Send right ghost cell
    MPI_Isend(&f[nxn_loc - 2], 1, MPI_DOUBLE, right, 1, MPI_COMM_WORLD, &requests[2]);
    // Receive left ghost cell
    MPI_Irecv(&f[0], 1, MPI_DOUBLE, left, 1, MPI_COMM_WORLD, &requests[3]);
