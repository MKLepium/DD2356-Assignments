...
int main(int argc, char *argv[]) {
    ...
    // Gather all values of f from all processes to rank 0
    double *all_f = NULL;
    if (rank == 0) {
        all_f = malloc(nxn_loc * size * sizeof(double));
    }
    MPI_Gather(f, nxn_loc, MPI_DOUBLE, all_f, nxn_loc, MPI_DOUBLE, 0, MPI_COMM_WORLD);

    // Print f values on rank 0
    if (rank == 0) {
