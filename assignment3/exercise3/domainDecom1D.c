...
int main(int argc, char *argv[]){
	...
	// exchange ghost cells using blocking communication
	// Left cell
	int left = (rank-1+size) % size;
	// Right cell
	int right = (rank+1) % size;
	// send / receive value of left cell 
	//MPI_Send(&f[1], 1, MPI_DOUBLE, left, 0, MPI_COMM_WORLD);
	//MPI_Recv(&f[0], 1, MPI_DOUBLE, left, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
	
	//MPI_Send(&f[nxn_loc-2], 1, MPI_DOUBLE, right, 1, MPI_COMM_WORLD);
	//MPI_Recv(&f[nxn_loc-1], 1, MPI_DOUBLE, right, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
	// exchange ghost cells using blocking communication
	MPI_Sendrecv(&f[1], 1, MPI_DOUBLE, left, 0, &f[nxn_loc-1], 1, MPI_DOUBLE, right, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
	MPI_Sendrecv(&f[nxn_loc-2], 1, MPI_DOUBLE, right, 1, &f[0], 1, MPI_DOUBLE, left, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
